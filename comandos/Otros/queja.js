...
const _queja = new Discord.MessageEmbed().setColor("#FDFDFD");
...
module.exports = {
    ...
    premium: false,
    alias: [],
    description: "¿Alguna queja sobre el bot?",
        ...
        collector.on('collect', async m => {
            ...
            if(m.author.id == message.author.id) {                    
                if(`${m.content}`.toLowerCase() == 'enviar') {
                    ...
                    client.channels.cache.get("YOUR-STAFF-LOGS-CHANNEL").send({ embeds: [ _queja.setTitle('Queja.').setAuthor(`${message.author.tag}, ${message.author.id}`, message.author.displayAvatarURL()).setFooter(`${message.guild.name}, ${message.guild.id}`, message.guild.iconURL) ] });
                    collector.stop();
                }else {
