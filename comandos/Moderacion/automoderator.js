...
module.exports = {
    ...
    try {
        ...
        if(!message.guild.members.me.permissions.has('ADMINISTRATOR'))return message.channel.send('Necesito permiso de __Administrador__.');
        ...
        if(args[0] == 'enable') {
            ...
            if(args[1] == 'warns') {
                ...
                message.reply({ embeds: [ new Discord.MessageEmbed().setColor("#FDFDFD").setDescription(`Después de este mensaje, escribe la cantidad de warns que debo agregarle a un usuario para mutearlo.`) ] });
                ...
                collector.on('collect', m => {
                    ...
                    if(m.author.id == message.author.id) {            
                        ...
                        message.reply({ embeds: [ new Discord.MessageEmbed().setColor("#FDFDFD").setDescription(`Ahora escribe la cantidad de warns que debo agregarle a un usuario para banearlo/expulsarlo del servidor.`) ] });
                        returnDetector = true;
                ...
                if(args[2] == 'add') {
                    ...
                    let cc = 1;
                    message.reply({ embeds: [ new Discord.MessageEmbed().setColor("#FDFDFD").setDescription(`Estás viendo los ${_guild.moderation.automoderator.actions.linksToIgnore.length} links que estoy ignorando actualmente, después de este mensaje escribe el número adjunto al link para eliminarlo.\n\n${_guild.moderation.automoderator.actions.linksToIgnore.map(x => `\`${cc++}-\` ${x}`).join('\n')}`) ] });
